...
//					}
	 ...
	 *****************************************/
        public void exportPNGmulti()
        {
        	...
        	int fileSaveOption = DropBoxSaver.getFileSaveOption();
        	Boolean isDropBoxUpload = fileSaveOption == JOptionPane.YES_OPTION;
		...
		if (result == JFileChooser.APPROVE_OPTION)
		{
            ...
            if(writeDown==true)
            {
                ...
                try
                {
                	...
                	String authCode = DropBoxSaver.showDropBoxAuthenticationDetails();
                	String authToken = DropBoxSaver.getAuthToken(authCode);
                    ...
                    for (int i = 0; i < imgs.length; i++)
                    {
                    	...
                    	if(isDropBoxUpload){
                        	DropBoxSaver.uploadFile(f, authToken);
                    	}
                    }    
                }
                catch(Exception e)
		...
		if (result == JFileChooser.APPROVE_OPTION)
		{
            ...
            if(writeDown==true)
            {
                ...
                try
                {
                    	...
                    	if(isDropBoxUpload){
                    	    DropBoxSaver.saveToDropBox(filename);
                    	}
                }
                catch(Exception e)
                {
...
//		if(nsdName.indexOf("(")>=0) {nsdName=nsdName.substring(0,nsdName.indexOf("("));}
	...
	public void count()
	{
		...
		elec.setZero();
		//		Counter counter = new Counter(NSDControl.getFrame());
		Element element = getSelected();
		EditData data = new EditData();
		data.title="Edit element ...";
		data.text.setText(element.getText().getText());
...
//		showInputBox(data, element.getClass().getSimpleName(), false);
		...
		CounterBox counterbox = new CounterBox(NSDControl.getFrame(), true);
		
		counterbox.setLocationRelativeTo(NSDControl.getFrame());

		// set title (as default)
		counterbox.setTitle("Counter");

		// set field
		// START KGU#43 2015-10-12: Breakpoint support
//		counterbox.chkBreakpoint.setEnabled(getSelected() != root);
//		counterbox.chkBreakpoint.setSelected(_data.breakpoint);
//		// END KGU#43 2015-10-12
...
//        }
	...
	public void searchNSD() {
		
				Search search = new Search(NSDControl.getFrame());
				Point p = getLocationOnScreen();
				search.setLocation(Math.round(p.x+(getVisibleRect().width-search.getWidth())/2+this.getVisibleRect().x),
										Math.round(p.y+(getVisibleRect().height-search.getHeight())/2+this.getVisibleRect().y));

				
				search.setLang(NSDControl.getLang());
				search.pack();
				search.setVisible(true);		
				//Element.preFor=search.edtFor.getText();
